...
export function StatCard({
  ...
  return (
    <article className="native-card native-press p-3 transition sm:p-4">
      <div className="flex items-center justify-between">
        <span className="grid h-7 w-7 place-items-center rounded-lg bg-[#1F5F3F]/10 text-[#1F5F3F] sm:h-9 sm:w-9">
      ...
      <div className="mt-2.5">
        <p className="font-display text-lg font-bold tracking-tight text-[#1C1D20] sm:text-2xl">
          {value}
        </p>
...
export function EmptyState({
  ...
  return (
    <div className="native-card border-dashed p-6 text-center sm:p-10">
      <span className="mx-auto grid h-10 w-10 place-items-center rounded-full bg-[#1F5F3F]/10 text-[#1F5F3F] sm:h-12 sm:w-12">
        <Icon className="h-5 w-5 sm:h-6 sm:w-6" />
      </span>
      <h3 className="mt-3 text-sm font-bold text-[#1C1D20] sm:text-base">{title}</h3>
      <p className="mx-auto mt-1 max-w-sm text-xs text-[#5F5E5E] sm:text-sm">{description}</p>
      {href && actionLabel ? (
        <Link
          href={href}
          className="native-press mt-4 inline-flex h-9 items-center rounded-lg bg-[#1F5F3F] px-3.5 text-xs font-semibold text-white shadow-xs transition hover:bg-[#164E33]"
        >
          {actionLabel}
        </Link>
      ) : null}
...
export function ActionCard({
  ...
  return (
    <Link
      href={href}
      className="native-card native-press group flex items-center gap-3 p-3 transition hover:border-[#1F5F3F]/30 hover:shadow-xs sm:p-4"
    >
      <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#1F5F3F]/10 text-[#1F5F3F] sm:h-10 sm:w-10">
        <Icon className="h-4 w-4 sm:h-5 sm:w-5" />
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-xs font-bold text-[#1C1D20] sm:text-sm">{title}</span>
